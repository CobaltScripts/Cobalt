...
internal class UIModeSetting(private val setting: ModeSetting) : UIComponent(
  ...
  override fun render() {
    ...
    if (isExpanded) {
      ...
      renderDropdown(dropdownX, buttonY + buttonHeight + 4F)
    }
  }

  private fun renderDropdown(dropdownX: Float, dropdownY: Float) {
    val dropdownWidth = calculateDropdownWidth()
    ...
    for ((index, option) in setting.options.withIndex()) {
      ...
      val textColor = if (isSelected) Color(61, 94, 149) else Color(200, 200, 200)
      NVGRenderer.text(
        option,
        dropdownX + 12F,
        optionY + 7F,
